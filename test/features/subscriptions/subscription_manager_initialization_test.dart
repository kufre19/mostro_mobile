  ...
  group('SubscriptionManager Initialization Fix - Code Verification', () {
    test('CRITICAL: _initializeExistingSessions() method must exist', () async {
      ...
      expect(content.contains('_initializeExistingSessions()'), isTrue,
          reason:
              'The _initializeExistingSessions() method call must exist in constructor');

      // Verify the method implementation exists
      expect(content.contains('void _initializeExistingSessions()'), isTrue,
          reason:
              'The _initializeExistingSessions() method implementation must exist');
      ...
      expect(
          content.contains('_updateAllSubscriptions(existingSessions)'), isTrue,
          reason:
              'The method must call _updateAllSubscriptions with existing sessions');
    });

    test('CRITICAL: constructor must call _initializeExistingSessions()',
        () async {
      final file = File('lib/features/subscriptions/subscription_manager.dart');
      final content = await file.readAsString();
      ...
      final constructorMatch =
          RegExp(r'SubscriptionManager\(this\.ref\)\s*\{([^}]+)\}')
              .firstMatch(content);
      ...
      expect(constructorBody.contains('_initializeExistingSessions()'), isTrue,
          reason:
              'Constructor must call _initializeExistingSessions() - THIS IS THE CRITICAL FIX');
    });
    ...
    test('CRITICAL: fireImmediately must remain false', () async {
      ...
      expect(content.contains('fireImmediately: false'), isTrue,
          reason:
              'fireImmediately: false must be preserved to prevent relay switching bug');
    });

    test(
        'CRITICAL: relay subscriptions preserved during session initialization',
        () async {
      final file = File('lib/features/subscriptions/subscription_manager.dart');
      final content = await file.readAsString();
      ...
      expect(
          content
              .contains('final filter = _createFilterForType(type, sessions)'),
          isTrue,
      ...
      expect(content.contains('if (filter == null)'), isTrue,
          reason:
              'Must check for null filter to prevent cancelling relay subscriptions');
      ...
      final updateMethodIndex = content.indexOf(
          'void _updateSubscription(SubscriptionType type, List<Session> sessions)');
      final filterCheckIndex =
          content.indexOf('if (filter == null)', updateMethodIndex);
      final unsubscribeAfterCheckIndex =
          content.indexOf('unsubscribeByType(type);', filterCheckIndex);

      expect(updateMethodIndex, greaterThan(-1),
          reason: '_updateSubscription method must exist');
      expect(filterCheckIndex, greaterThan(updateMethodIndex),
          reason: 'Filter null check must exist after method start');
      expect(unsubscribeAfterCheckIndex, greaterThan(filterCheckIndex),
          reason: 'unsubscribe must happen after filter null check');
    });
  });
}
