...
void main() {
  group('NostrUtils mostro URL tests', () {
    ...
    test('should reject invalid mostro URLs', () {
      ...
      expect(
          NostrUtils.isValidMostroUrl('mostro:order123'), isFalse); // no relays
      expect(NostrUtils.isValidMostroUrl('mostro:?relays=wss://relay.com'),
          isFalse); // no order id
      expect(
          NostrUtils.isValidMostroUrl('nostr:order123?relays=wss://relay.com'),
          isFalse); // wrong scheme
      expect(
          NostrUtils.isValidMostroUrl(
              'mostro:order123?relays=http://relay.com'),
          isFalse); // invalid relay protocol
    });
    ...
    test('should handle single relay in mostro URL', () {
      const singleRelayUrl =
          'mostro:order789?relays=wss://relay.mostro.network';
    ...
    test('should handle relays with spaces', () {
      const spacedRelaysUrl =
          'mostro:order456?relays=wss://relay1.com, wss://relay2.com , wss://relay3.com';
      final result = NostrUtils.parseMostroUrl(spacedRelaysUrl);

      expect(result, isNotNull);
      expect(result!['relays'],
          equals(['wss://relay1.com', 'wss://relay2.com', 'wss://relay3.com']));
    });
