...
class CantDoNotificationMapper {
  ...
  static final _messageMap = <String, String Function(BuildContext)>{
    ...
    'out_of_range_sats_amount': (context) =>
        S.of(context)!.outOfRangeSatsAmount,
    'cant_create_user': (context) => S.of(context)!.cantCreateUser,
    'out_of_range_fiat_amount': (context) =>
        S.of(context)!.outOfRangeFiatAmount,
  ...
  static String getMessage(BuildContext context, String cantDoReason) {
    ...
    _logger.w(
        'Unhandled cant-do reason: $cantDoReason. Consider adding to CantDoNotificationMapper.');

    // Fallback to generic cant-do message
    return NotificationMessageMapper.getLocalizedTitle(
        context, mostro.Action.cantDo);
  }
}
...
class NotificationListenerWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ref.listen<TemporaryNotification>(currentTemporaryNotificationProvider,
        (previous, next) {
      if (next.show) {
        String message;
        ...
        if (next.customMessage != null) {
          ...
          if (next.action == mostro.Action.cantDo) {
            ...
            if (reason is String && reason.isNotEmpty) {
              ...
              message = NotificationMessageMapper.getLocalizedTitle(
                  context, mostro.Action.cantDo);
            }
          } else {
