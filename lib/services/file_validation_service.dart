...
class FileValidationService {
  ...
  static Future<void> _performTypeSpecificValidation(
      Uint8List fileData, String mimeType, String fileType) async {
    switch (fileType) {
      case 'document':
  ...
  static Future<void> _validatePdfStructure(Uint8List fileData) async {
    ...
    final tail =
        String.fromCharCodes(fileData.skip(fileData.length - 20).take(20));
  ...
  static Future<void> _validateDocumentStructure(
    ...
    if (mimeType == 'application/msword') {
      ...
      if (_containsMacroPatterns(fileData)) {
        throw FileValidationException(
            'Document contains macros which are not allowed for security reasons');
      }
    } else if (mimeType ==
        'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      ...
      if (fileData[0] != 0x50 || fileData[1] != 0x4B) {
        throw FileValidationException(
            'Invalid DOCX file format (not a ZIP archive)');
      }
  ...
  static Future<void> _validateVideoStructure(
      Uint8List fileData, String mimeType) async {
